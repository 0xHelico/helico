...
|---|---|---|---|
| Chainlink CRE | reusable package: the enclave recomputes the mandate hash, reads the vault and the pool from inside the TEE, mirrors the vault's range rule, sizes the swap and the mint, and encodes `(act, mandateHash, RecenterParams)` for the vault; unit-tested, decision simulated in the CRE simulator on an earlier binary; **not run against a deployed vault, not deployed** | [`packages/plugins/cre/`](packages/plugins/cre/) | [`src/index.ts#L161-L196`](packages/plugins/cre/src/index.ts#L161-L196) `onCronTrigger` runs in the enclave, [`src/index.ts#L199-L217`](packages/plugins/cre/src/index.ts#L199-L217) `deliver` writes the report, [`src/index.ts#L220-L228`](packages/plugins/cre/src/index.ts#L220-L228) `initWorkflow` registers it with `handlerInTee` |
