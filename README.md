...
|---|---|---|---|
| Chainlink CRE | reusable package: the enclave recomputes the mandate hash, reads the pool tick from inside the TEE, and reports only the re-centre verdict; unit-tested and **simulated in the CRE simulator against Robinhood Chain Testnet** (results in the package README); **not wired into an app, not deployed** | [`packages/plugins/cre/`](packages/plugins/cre/) | [`src/index.ts#L98-L135`](packages/plugins/cre/src/index.ts#L98-L135) `onCronTrigger` runs in the enclave, [`src/index.ts#L138-L146`](packages/plugins/cre/src/index.ts#L138-L146) `initWorkflow` registers it with `handlerInTee`, [`src/mandate.ts`](packages/plugins/cre/src/mandate.ts) hash, [`src/decision.ts`](packages/plugins/cre/src/decision.ts) rules |
